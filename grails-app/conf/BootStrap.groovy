...
class BootStrap {
  ...
  def init = { servletContext ->
    ...
    if (!SecUser.count()) {
      ...
      new Note(category: ctn2, name: 'Athene', value: 'мелкие и средней величины птицы семейства совиных. Отличительный признак — отсутствие пучков перьев («ушек ').save(failOnError: true)
      new Note(category: ctn2, name: 'звук среднего ряда', value: 'его отмечают как «zпсһ» в ряду с другими названиями сов. В славянских языках слово звучит довольно схоже: в').save(failOnError: true)
      new Note(category: ctn2, name: 'Последнее изменение', value: 'ряд ли приемлемы. Среди вокализаций сычей шипение, пожалуй, один из самых редких звуков, который птица издает лишь в крайней степени испуга. Свидетельством этого может служить, например, звукоподражатель').save(failOnError: true)
      new Note(category: ctn2, name: 'Верхний подъём', value: 'плане название «сыч» уместно сблизить со словом «суслик, сусел» — названием зверька, для которого свист так же основная форма вокализ').save(failOnError: true)
      new Note(category: ctn2, name: 'в безударной позиции', value: 'Таким образом, название «сыч» возникло как звукоподражательное и в буквальном смысле значит «свистун»[1').save(failOnError: true)
      new Note(category: ctn2, name: 'Эти особенности', value: 'В отличие от большинства славянских языков, в болгарском языке падежи почти исчезли, зато используются определённый, неопределённый[1] и так называем ').save(failOnError: true)
      new Note(category: ctn2, name: 'румынами ап', value: 'Лексически болгарский язык довольно близок к церковнославянскому и до сих пор содержит множество слов,  ').save(failOnError: true)
      new Note(category: ctn2, name: 'грамматической традиции', value: '«-лар-» и «-джия-»), сказалось в основном в лексике. Наконец, ускорению процесса разложения старого синтетического строя болгарского языка и возникновению').save(failOnError: true)
      new Note(category: ctn2, name: 'Новоболгарский', value: 'бесправном положении («райя»). Это, при наличии преследования литературного болгарского языка, должно было выдвинуть на первый п').save(failOnError: true)
